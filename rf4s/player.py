...
class Player:
    ...
    def _update_tackle(self) -> None:
        available, candidates = self._is_any_tackle_available()
        if not available:
        ...
        if self.cfg.SCRIPT.RANDOM_ROD_SELECTION:
            ...
            self.tackle_idx = (candidates.index(self.tackle_idx) + 1) % len(candidates)
        self.tackle = self.tackles[self.tackle_idx]

    def _is_any_tackle_available(self) -> bool:
        if self.num_tackle == 1 and self.tackle.available:
            return True, [self.tackle]

        #  Exclude current rod only if there's another available tackle
        candidates = [i for i, tackle in enumerate(self.tackles) if tackle.available]
    ...
    def _change_tackle_pva(self) -> None:
        if self.cfg.ARGS.PVA and self.detection.is_pva_not_chosen():
            self.tackle.change_pva()
    ...
    def start_bottom_mode(self) -> None:
        ...
        while True:
            ...
            pag.press(str(self.cfg.KEY.RODS[self.tackle_idx]))
    ...
    def _refill_dry_mix(self) -> None:
        if self.cfg.ARGS.DRY_MIX:
            self.tackle.change_dry_mix()
        else:
            logger.warning("Dry mix auto-refill is disabled")
            pag.press("0") # Put down spod rod to skip casting
    ...
    def _refill_ground_bait(self):
        """Refill ground bait."""
        if not self.cfg.ARGS.GROUNDBAIT:
            return
        logger.info("Refilling ground bait")
        pag.press("v")
        sleep(ANIMATION_DELAY)
        scrollbar_position = self.detection.get_scrollbar_position()
        if scrollbar_position is None:
            slot = self.detection.get_groundbait_slot_position()
            print(slot)
            if slot is None:
                logger.warning("Groundbait slot is not empty or disabled")
                return
            pag.moveTo(slot)
            pag.click()
            sleep(ANIMATION_DELAY)
            self._replace_item(lure=False)
        pag.press("v")